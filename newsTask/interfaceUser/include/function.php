...
function innerJoin($var){
    global $conn;
    $stmt = $conn->prepare("SELECT c.cate_id , c.name_category, a.art_id, a.title , a.image ,a.description FROM category c  INNER JOIN artical a ON c.cate_id  =  a.category_id WHERE cate_id =?");
    $stmt->execute(array($var));
    $data = $stmt->fetchAll();
    return $data;

}
function lastfive($table){
    // $GLOBALS['conn'];
    global $conn;
    $stmt = $conn->prepare("SELECT * FROM $table ORDER BY title ,description,image DESC LIMIT 5");
    $stmt->execute();
    $data = $stmt->fetchAll();
    return $data;
}
